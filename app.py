...
if upload_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        with col3:
            st.metric(label="Media Shared",value=number_media_messages)
        with col4:
        ...
        with col1:
            ...
            fig,ax=plt.subplots()
            ax.bar(busy_day.index,busy_day.values,color="#00CC96")
            plt.xticks(rotation="vertical")
            st.pyplot(fig)
        with col2:
        ...
        if selected_user=="Overall":
            ...
            col1,col2=st.columns(2)
            with col1:
                ax.bar(x.index,x.values,color="#145A5A")
                plt.xticks(rotation="vertical")
                st.pyplot(fig)
            with col2:
                st.dataframe(new_df)
        ...
        fig,ax=plt.subplots()
        ax.barh(most_common_df[0],most_common_df[1],color="#F15BB5")
        plt.xticks(rotation="vertical")
